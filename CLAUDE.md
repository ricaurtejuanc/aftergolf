...
- **Client data-access layer**: one file per domain in `src/lib/` (`courseStore.ts`, `holeScoreStore.ts`, `productStore.ts`, `shopSettingsStore.ts`, `storage.ts` for rounds) — each defines a `*Row` interface matching the DB's snake_case columns, a `fromRow()` mapper to the camelCase domain type in `src/data/*.ts`, and `loadX`/`addX`/`updateX`/`deleteX` functions. Mutations don't return the single changed row — they re-run `loadX()` and return the full fresh list, which callers just `setState` with. Manual reordering (`reorderProduct`, `reorderColorPhoto`, `reorderProductColor`) works by swapping a `position` column between two rows.
- **Manually-uploaded photos** (Shop product images) live in the `product-mockups` Storage bucket under `manual/<id>/<slug>/<random>` — the random suffix means a failed upload can never overwrite a photo that's already showing, and `isManualPhoto()` in `productStore.ts` checks the URL prefix to distinguish an admin-uploaded photo from a Printful CDN photo when deciding what a Printful reimport is allowed to overwrite. `src/lib/imageOptimize.ts` downscales (1600px max dimension) and re-encodes to WebP via canvas before every new upload; `optimizeExistingPhotos()` in `productStore.ts` is an admin-triggered one-off (a panel in `ProductsAdminPage.tsx`) that recompresses manual photos already in Storage — both are scoped to `isManualPhoto()` so Printful's own CDN mockups are never touched.
- **Shop checkout**: no payment gateway — `bizum-order`/`confirm-bizum-order` edge functions record an order and the customer pays by Bizum transfer, confirmed manually by the admin in the Pedidos tab. `ORDERS_ENABLED` (`src/context/CartContext.tsx`) is a hardcoded kill switch — when `false`, `CartPanel.tsx` still shows cart contents/totals but replaces the address form with a "coming soon" message. `shop_settings` is a single-row table (shipping cost, free-shipping threshold) editable from the admin's "Envío" panel; the `bizum-order` function reads the same row server-side so the checkout total always matches what's configured.

### Course/tee data model and scorecards

Golf courses are three levels deep: `courses` → `recorridos` (a club's distinct 18-hole layouts, e.g. Real Club Puerta de Hierro's "Abajo"/"Buenavista"/"Arriba" — most clubs have exactly one) → `tees` (blanco/amarillo/rojo, each with CR/Slope/Par) → `hole_scores` (one row per hole per tee: distance in meters, par, stroke index/hcp). `courseStore.ts`'s `loadCourses()` nests all three levels into `GolfCourse.rounds[].tees[]`; `holeScoreStore.ts`'s `loadHoleScores(teeId)` is fetched separately, on demand, once a specific tee is selected.

`src/components/CourseTeeSelect.tsx` is the shared picker used by both `/antes-de-jugar` and `/despues-de-jugar`: Campo → Recorrido (auto-skipped when a course has only one) → Tee. A course not in the local list falls back to a GolfCourseAPI search (`GolfCourseApiFallback` in the same file) — that path has no recorrido concept, so its result is wrapped into a single synthetic round to reuse the same rendering logic. Once a tee with scorecard data is selected, a "Ver tarjeta" button opens the 18-hole card (with an Ida/Vuelta/Todos filter); it simply doesn't render for a tee with no `hole_scores` rows (API-sourced tees, or a local tee the admin hasn't filled in yet) — there's no separate "not available" state to handle.

`AntesDeJugarPage.tsx` has its own second scorecard button, "Ver hoyos con punto" (`HolesWithStrokesModal`, page-local rather than part of the shared picker, since it depends on that page's own player/handicap state): the same 18-hole table with one column per player marking which holes they get a handicap stroke on, via `strokesOnHole()` in `lib/handicap.ts` (standard WHS stroke-index allocation — a player receiving N strokes gets one extra stroke on each of the N hardest holes). With 2+ players the allocation is relative to the group's lowest handicap (mirrors "Distribuir Handicap"'s own math, scratch player gets none); with a single player it falls back to their own full course handicap, since there's no one to play off.
...
`LanguageContext.tsx` (mirrors `AuthContext`/`CartContext`) holds the active language (`es`/`en`, persisted to `localStorage`, defaulting to `es` with no browser-locale auto-detection) and exposes `dict`, the active language's nested dictionary from `src/i18n/es.ts`/`en.ts`. `en.ts` is typed against `es.ts`'s exact shape (leaves widened to `string`), so a missing or extra key in either file fails the build — read as `dict.section.key` directly rather than a string-path lookup, for compile-time safety. **The admin panel is deliberately out of scope** — `es.ts`'s own comment states it's used by nobody but the single Spanish-speaking site owner, so `CoursesPage.tsx`/`ProductsAdminPage.tsx`/etc. are plain hardcoded Spanish, not wired to `useLanguage()`.

### Handicap calculations (`src/lib/handicap.ts`)

Pure functions, no React/Supabase dependencies — `calculateCourseHandicap` (pre-round: HI → Course Handicap for a given tee), `calculateRoundResult` (post-round: gross score → net score, Stableford points, Score Differential), and `strokesOnHole` (which holes a given stroke allocation lands on). Covered by `src/lib/handicap.test.ts` against a known real round; add new cases there rather than asserting against invented numbers.
