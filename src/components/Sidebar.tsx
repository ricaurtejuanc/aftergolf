...
function navItems(dict: typeof es) {
  return [
    { to: '/antes-de-jugar', label: dict.sidebar.antesDeJugar, icon: FlagIcon },
    { to: '/despues-de-jugar', label: dict.sidebar.despuesDeJugar, icon: ScoreIcon },
    { to: '/historial', label: dict.sidebar.historial, icon: HistoryIcon },
    { to: '/shop', label: dict.sidebar.shop, icon: ShopIcon },
    { to: '/contacto', label: dict.sidebar.contacto, icon: MailIcon },
  ]
}
...
function LanguageSwitch({ className = '' }: { className?: string }) {
  ...
  function optionClass(option: Lang) {
    return `rounded px-1.5 py-0.5 text-[10px] font-semibold transition ${
      lang === option ? 'bg-fairway-800 text-cream-50' : 'text-fairway-500 hover:text-fairway-800'
    }`
  }

  return (
    <div className={`flex items-center gap-0.5 rounded-lg border border-cream-300 bg-white p-0.5 ${className}`}>
      <button type="button" onClick={() => setLang('es')} className={optionClass('es')}>
        ES
      </button>
      <button type="button" onClick={() => setLang('en')} className={optionClass('en')}>
        EN
      </button>
    </div>
  )
    ...
    <>
      <header className="relative flex items-center justify-between border-b border-cream-300 bg-cream-100 px-4 py-3 md:hidden">
        ...
        <div className="flex min-w-0 items-center gap-2">
          ...
          >
            {dict.sidebar.handicapCta}
          </NavLink>
          <button
            type="button"
            onClick={() => setCartOpen((v) => !v)}
            aria-label={dict.sidebar.viewCart}
      ...
      <aside className="hidden w-64 shrink-0 border-r border-cream-300 bg-cream-100 md:sticky md:top-0 md:flex md:h-screen md:flex-col md:overflow-y-auto">
        ...
        <div className="mt-auto p-3">
          ...
          >
            {dict.sidebar.handicapCta} <span className="font-semibold">{dict.sidebar.handicapCtaLink}</span>
          </NavLink>
        </div>
      </aside>
    </>
